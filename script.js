...
var cursorLens = document.querySelector('.cursor-lens');
...
// Only run cursor if elements exist on this page
if (cursorDot && cursorLens) {

  // Snap cursor to centre so they never appear stuck at 0,0
  ...
  function animateCursor() {
    var distX = mouseX - lensX;
    var distY = mouseY - lensY;
    lensX += distX * 0.12;
    lensY += distY * 0.12;
    cursorLens.style.left = lensX + 'px';
    cursorLens.style.top  = lensY + 'px';
  ...
  /* --- HOVER DETECTION & CURSOR SCALING --- */
  function attachHoverEffects() {
    var interactables = document.querySelectorAll('a, button, .nav-links span, .square-card, .toggle-password, .result-mini-card');
  ...
  /* DETECT IF HOVERING A FORM / INPUT AREA — switch to system cursor */
  function attachFormModeDetection() {
    // Auth card (login/signup)
    var authCard = document.querySelector('.auth-card');
    if (authCard) {
      authCard.addEventListener('mouseenter', function() { document.body.classList.add('form-mode'); });
      authCard.addEventListener('mouseleave', function() { document.body.classList.remove('form-mode'); });
    }

    // Admin add-user form area
    var adminAddForm = document.querySelector('.admin-add-form');
    if (adminAddForm) {
      adminAddForm.addEventListener('mouseenter', function() { document.body.classList.add('form-mode'); });
      adminAddForm.addEventListener('mouseleave', function() { document.body.classList.remove('form-mode'); });
    }

    // Settings form areas
    var settingsForms = document.querySelectorAll('.settings-form');
    settingsForms.forEach(function(form) {
      form.addEventListener('mouseenter', function() { document.body.classList.add('form-mode'); });
      form.addEventListener('mouseleave', function() { document.body.classList.remove('form-mode'); });
    });

    // Country search card
    var searchCard = document.querySelector('.search-card');
    if (searchCard) {
      searchCard.addEventListener('mouseenter', function() { document.body.classList.add('form-mode'); });
      searchCard.addEventListener('mouseleave', function() { document.body.classList.remove('form-mode'); });
    }

    // Any <input>, <textarea>, <select> directly
    ...
    document.addEventListener('mouseout', function(e) {
      ...
      if (tag === 'INPUT' || tag === 'TEXTAREA' || tag === 'SELECT') {
        ...
        if (!rel || (rel.tagName !== 'INPUT' && rel.tagName !== 'TEXTAREA' && rel.tagName !== 'SELECT')) {
          document.body.classList.remove('form-mode');
        }
      }
    });
  }
  attachFormModeDetection();
}
...
function showToast(message, type) {
  type = type || 'error';
  var existing = document.querySelector('.lustre-toast');
  if (existing) existing.remove();
